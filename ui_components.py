...
def update_output(text, is_error=False):
    ...
    try:
        ...
        if not app_state or not app_state.output_text:
            logger.error("Output widget not available")
            return
        ...
        if is_error:
            app_state.output_text.tag_add("error", "1.0", "end")
            app_state.output_text.tag_configure("error", foreground="red")

        app_state.output_text.configure(state="disabled")
        app_state.output_text.see("end")
...
def clear_output(output_text=None):
    ...
    if output_text:
        ...
        try:
            from main import app_state
            if app_state and app_state.output_text:
                app_state.output_text.configure(state="normal")
                app_state.output_text.delete("1.0", "end")
                app_state.output_text.configure(state="disabled")
        except Exception as e:
...
def copy_to_clipboard(output_text=None):
    ...
    if output_text:
        ...
        try:
            from main import app_state
            if app_state and app_state.output_text:
                text = app_state.output_text.get("1.0", "end").strip()
                app_state.output_text.clipboard_clear()
                app_state.output_text.clipboard_append(text)
                app_state.output_text.update()
        except Exception as e:
...
def save_output(output_text=None):
    ...
    if output_text:
        ...
        try:
            ...
            logger.error(f"Failed to get output text: {e}")
